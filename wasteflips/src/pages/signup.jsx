import React from "react";
import { Link } from "react-router-dom";
import "./signup.css";

const LeftContent = ({ text, description }) => (
  <>
    <h2>{text}</h2>
    <p>{description}</p>
  </>
);
// Reusable input field component
const InputField = ({ type, id, name, placeholder, label }) => (
  <fieldset className={`${id}-field`}>
    <legend>{label}</legend>
    <input type={type} id={id} name={name} placeholder={placeholder} required />
  </fieldset>
);

// Reusable social button component
const SocialButton = ({ platform, link, children }) => (
  <button className={`social-btn ${platform}`}>
    <Link to={link} className={`${platform}-link`}>
      {children}
    </Link>
  </button>
);

const SignupPage = () => {
  return (
    <div className="signup-container">
      <div className="body-ct">
        <header className="title">WasteFlip</header>
        <div className="form-container">
          <LeftContent
            text="Sign up"
            description="Let's get you all set up so you can access your personal account"
          />

          <form action="#">
            <div className="form-ct">
              <InputField
                type="text"
                id="full-name"
                name="full-name"
                label="Full Name"
                placeholder="Ann Pine"
              />

              <div className="E-P">
                <InputField
                  type="email"
                  id="email"
                  name="email"
                  placeholder="ann.pine@gmail.com"
                  label="Email"
                />
                <InputField
                  type="tel"
                  id="phone"
                  name="phone"
                  placeholder="233 034 3456 578"
                  label="Phone Number"
                />
              </div>

              <InputField
                type="password"
                id="password"
                name="password"
                label="Password"
                placeholder=""
              />
              <InputField
                type="password"
                id="confirm-password"
                name="confirm-password"
                label="Confirm Password"
                placeholder=""
              />
            </div>

            <div className="terms-conditions">
              <input type="checkbox" id="terms" name="terms" required />
              <label htmlFor="terms">
                I agree to the{" "}
                <Link to="/terms" className="T">
                  Terms
                </Link>{" "}
                and{" "}
                <Link to="/policy" className="P">
                  Policy
                </Link>
              </label>
            </div>

            <div className="signup-actions">
              <Link to="/homepage">
                <button type="submit">Create Account</button>
              </Link>
              <div className="login-link">
                <p>
                  Already have an account?{" "}
                  <Link to="/login" className="login-sty">
                    Login
                  </Link>
                </p>
              </div>
            </div>
          </form>

          <div className="other-signin">
            <div className="or-sign">
              <p>Or Sign up with</p>
            </div>

            <div className="social">
              <SocialButton platform="facebook" link="#">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="1em"
                  height="1em"
                  viewBox="0 0 256 256"
                >
                  <path
                    fill="#1877f2"
                    d="M256 128C256 57.308 198.692 0 128 0S0 57.308 0 128c0 63.888 46.808 116.843 108 126.445V165H75.5v-37H108V99.8c0-32.08 19.11-49.8 48.348-49.8C170.352 50 185 52.5 185 52.5V84h-16.14C152.959 84 148 93.867 148 103.99V128h35.5l-5.675 37H148v89.445c61.192-9.602 108-62.556 108-126.445"
                  />
                  <path
                    fill="#fff"
                    d="m177.825 165l5.675-37H148v-24.01C148 93.866 152.959 84 168.86 84H185V52.5S170.352 50 156.347 50C127.11 50 108 67.72 108 99.8V128H75.5v37H108v89.445A129 129 0 0 0 128 256a129 129 0 0 0 20-1.555V165z"
                  />
                </svg>
              </SocialButton>
              <SocialButton platform="google" link="#">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="0.98em"
                  height="1em"
                  viewBox="0 0 256 262"
                >
                  <path
                    fill="#4285f4"
                    d="M255.878 133.451c0-10.734-.871-18.567-2.756-26.69H130.55v48.448h71.947c-1.45 12.04-9.283 30.172-26.69 42.356l-.244 1.622l38.755 30.023l2.685.268c24.659-22.774 38.875-56.282 38.875-96.027"
                  />
                  <path
                    fill="#34a853"
                    d="M130.55 261.1c35.248 0 64.839-11.605 86.453-31.622l-41.196-31.913c-11.024 7.688-25.82 13.055-45.257 13.055c-34.523 0-63.824-22.773-74.269-54.25l-1.531.13l-40.298 31.187l-.527 1.465C35.393 231.798 79.49 261.1 130.55 261.1"
                  />
                  <path
                    fill="#fbbc05"
                    d="M56.281 156.37c-2.756-8.123-4.351-16.827-4.351-25.82c0-8.994 1.595-17.697 4.206-25.82l-.073-1.73L15.26 71.312l-1.335.635C5.077 89.644 0 109.517 0 130.55s5.077 40.905 13.925 58.602z"
                  />
                  <path
                    fill="#eb4335"
                    d="M130.55 50.479c24.514 0 41.05 10.589 50.479 19.438l36.844-35.974C195.245 12.91 165.798 0 130.55 0C79.49 0 35.393 29.301 13.925 71.947l42.211 32.783c10.59-31.477 39.891-54.251 74.414-54.251"
                  />
                </svg>
              </SocialButton>
              <SocialButton platform="apple" link="#">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="1em"
                  height="1em"
                  viewBox="0 0 24 24"
                >
                  <path
                    fill="black"
                    d="M17.05 20.28c-.98.95-2.05.8-3.08.35c-1.09-.46-2.09-.48-3.24 0c-1.44.62-2.2.44-3.06-.35C2.79 15.25 3.51 7.59 9.05 7.31c1.35.07 2.29.74 3.08.8c1.18-.24 2.31-.93 3.57-.84c1.51.12 2.65.72 3.4 1.8c-3.12 1.87-2.38 5.98.48 7.13c-.57 1.5-1.31 2.99-2.54 4.09zM12.03 7.25c-.15-2.23 1.66-4.07 3.74-4.25c.29 2.58-2.34 4.5-3.74 4.25"
                  />
                </svg>
              </SocialButton>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};
export default SignupPage;
export { LeftContent, SocialButton, InputField };
